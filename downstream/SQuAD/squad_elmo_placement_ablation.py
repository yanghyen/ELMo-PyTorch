#!/usr/bin/env python3
"""
SQuAD ELMo placement ablation (Table-3 style):
- input_only
- input_output
- output_only

Runs all settings and saves dev EM/F1 to CSV.
"""
...
DEFAULT_OUTPUT_CSV = Path(__file__).resolve().parent / "placement_ablation_metrics.csv"
...
class PlacementELMoQA(nn.Module):
    def __init__(self, bilm: nn.Module, hidden_dim: int, num_layers: int, placement: str) -> None:
        super().__init__()
        self.placement = placement
        self.elmo = ELMoEmbedding(bilm=bilm, num_layers=num_layers)
        self.base_q_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.base_c_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
        ...
        self.out_q_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.out_c_proj = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.start_head = nn.Linear(hidden_dim, 1)
        self.end_head = nn.Linear(hidden_dim, 1)

    @staticmethod
    def _bi_attention(c_h: Tensor, q_h: Tensor, c_mask: Tensor, q_mask: Tensor, c_proj: nn.Linear, q_proj: nn.Linear) -> Tensor:
        sim = torch.bmm(c_proj(c_h), q_proj(q_h).transpose(1, 2)) / (float(c_h.size(-1)) ** 0.5)
        ...
        return torch.cat([c_h, c2q, c_h * c2q, c_h * q2c], dim=-1)

    def forward(self, question_char_ids: Tensor, context_char_ids: Tensor) -> Tuple[Tensor, Tensor]:
        q_elmo = self.elmo(question_char_ids)
        c_elmo = self.elmo(context_char_ids)
        q_mask = token_mask_from_char_ids(question_char_ids)
        c_mask = token_mask_from_char_ids(context_char_ids)

        if self.placement in ("input_only", "input_output"):
            fused = self._bi_attention(c_elmo, q_elmo, c_mask, q_mask, self.base_c_proj, self.base_q_proj)
        elif self.placement == "output_only":
            # weak input path without ELMo-style feature interactions
            fused = torch.cat([c_elmo, c_elmo, c_elmo, c_elmo], dim=-1)
        else:
            raise ValueError(f"Unknown placement: {self.placement}")

        modeled, _ = self.modeling(fused)

        if self.placement in ("output_only", "input_output"):
            # output-level ELMo reinjection (acts like output-side ELMo use)
            out_features = self._bi_attention(modeled + c_elmo, q_elmo, c_mask, q_mask, self.out_c_proj, self.out_q_proj)
            modeled = out_features[..., : modeled.size(-1)]
...
def train_one_setting(
    ...
    for _ in range(epochs):
        ...
        for batch in train_loader:
            ...
            loss.backward()
            optimizer.step()
    dev_em, dev_f1 = evaluate(model, dev_loader, device)
    print(f"{setting_name}: dev_EM={dev_em:.4f}, dev_F1={dev_f1:.4f}")
    return dev_em, dev_f1


def append_result(csv_path: Path, setting: str, dev_em: float, dev_f1: float) -> None:
    csv_path.parent.mkdir(parents=True, exist_ok=True)
    need_header = not csv_path.is_file() or csv_path.stat().st_size == 0
    cols = ("timestamp", "task", "setting", "dev_em", "dev_f1")
    row = {
...
def run_squad_placement_ablation(
    ...
    _, num_layers, hidden_dim, options = load_pretrained_char_bilm_from_checkpoint(checkpoint_path, dev)
    ...
    for setting in ("input_only", "input_output", "output_only"):
        bilm = load_pretrained_char_bilm_from_checkpoint(checkpoint_path, dev)[0].to(dev)
        model = PlacementELMoQA(bilm=bilm, hidden_dim=hidden_dim, num_layers=num_layers, placement=setting).to(dev)
        em, f1 = train_one_setting(setting, model, train_loader, dev_loader, dev, epochs, lr)
        append_result(output_csv, setting, em, f1)

    print(f"Saved placement ablation metrics to: {output_csv}")
